...
describe("Tree", () => {
  describe("Basic nesting", () => {
    ...
    test("should create a tree with nested children", () => {
      const tree = new Tree("root", [
        new Tree("child1", []),
        new Tree("child2", [new Tree("grandchild1", [])]),
      ]);
      expect(tree.name).toBe("root");
      expect(tree.children).toEqual([
        new Tree("child1", []),
    ...
    test("should insert a child at a specific point", () => {
      ...
      expect(tree.children).toEqual([
        ...
        new Tree("child2", [
          ...
          new Tree("grandchild2", [new Tree("grandgrandchild1", [])]),
        ]),
      ]);
    });
  });
  describe("Parsing", () => {
    test("should parse a simple tree", () => {
      const tree = new Tree(ROOT_NAME, [
        new Tree("child1", []),
        new Tree("child2", [
          new Tree("grandchild1", [new Tree("grandgrandchild1", [])]),
        ]),
      ]);
